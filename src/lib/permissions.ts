...
export function canDo(
  ...
  switch (action) {
    ...
    case "receive":
      /**
       * 신청(2) → 진행(3). 처리자 측이면 누구나 접수할 수 있다 —
       * **접수 = 인계**이기 때문이다. 접수하는 순간 담당자가 접수한 사람으로 바뀐다
       * (mutations.ts). 그래서 아래 단계(save·propose·complete)의 '담당자만' 조건과
       * 어긋나지 않는다 — 접수한 사람이 곧 담당자다.
       *
       * ⚠️ 남의 배정을 가져오는 일이라 **이력에 누구에게서 가져왔는지 남긴다.**
       */
