...
저장(신청 등록·상태 변경·댓글·첨부)은 **로컬에서 바로 됩니다**. 라이브 데모는 서버리스라
인스턴스마다 메모리 DB 가 따로여서(저장해도 다음 조회에서 사라진다) 잠가 두었습니다 —
읽기와 권한 판정까지만 보여 줍니다. 강제로 열려면 `.env.local` 에 `ALLOW_DEV_WRITES=true` 를
넣고 재시작하세요 (공유 DB 를 붙였을 때를 위한 탈출구입니다).
