...
| | |
|---|---|
| **규모** | 화면 8개 + 알림센터 · TypeScript 16,800줄(+테스트 6,300줄) · 설계 기록(ADR) 12건 |
| **스택** | Next.js 16(App Router) · React 19 · TypeScript strict · Tailwind 4 + 자체 토큰 · Radix UI · SQLite/libSQL · Vitest · GitHub Actions · Vercel |
| **검증** | 테스트 366개 · lint → typecheck → test → build → 런타임 스모크 → 반응형 검사 자동 실행 |
| **측정** | Lighthouse 접근성·권장사항·SEO **측정한 6개 화면 모두 100** · 데스크톱 성능 97~99 |
...
컴포넌트 라이브러리를 가져다 쓰지 않고 **토큰 168개 · UI 프리미티브 12종 · 컴포넌트 클래스
...
고친 뒤 **측정한 6개 화면**(대시보드·조회·보드·공지·고객사·스타일가이드)이 모두
**Lighthouse 접근성 100 · 권장사항 100 · SEO 100**입니다 (`npm run measure` · `docs/measurements/summary.md`).
...
### 6. 포털 밖에서 오는 일도 포털 안으로

실제 유지보수는 포털 신청만으로 들어오지 않습니다 — 전화로 오고, 메일로 오고, 월 백업 확인·
세법 패치처럼 **고객사가 발의하지 않는 업무**도 있습니다. 이 건들이 밖에 있으면 보드가 실제
업무의 일부만 보여 줍니다.

업무 현황에서 **대신 등록**할 수 있게 하되, 출처(전화·메일·내부)와 성격(문의/작업)을 함께
남깁니다. 대리 등록은 **승인 단계를 타지 않습니다** — 대기 상태는 고객사 승인권자만 풀 수
있어서, 우리가 받아 적은 건을 그 줄에 세우면 아무도 진행시킬 수 없는 티켓이 됩니다.

매달 반복되는 업무는 템플릿으로 저장해 두고 **버튼 한 번으로 이번 달 회차**를 만듭니다.
서버리스라 스케줄러가 없어 사람이 방아쇠를 당기지만, 생성월을 티켓과 한 트랜잭션으로 기록해
여러 번 눌러도 한 건이고, 아직 안 만든 건수는 보드 버튼에 배지로 뜹니다.
...
데이터 프로파일)와 설계 결정 기록(ADR 12건)은 `docs/` 에 있습니다.

## 검증 · 배포

- **CI** (GitHub Actions): push/PR 마다 `verify`(lint·typecheck·테스트 366개) → `format:check` →
  `build` → **런타임 스모크**(주요 경로 — 화면 8 · API 2 — 와 `/api/diag`, 없는 경로의 한국어 404) → **반응형 검사**(390·768·1440 으로
...
- **라우트 핸들러 테스트** — 버튼을 숨기는 것은 표시일 뿐이고 요청을 막는 것은 라우트입니다.
  화면을 거치지 않고 직접 호출해도 거부되는지(타사 티켓 404·취소는 신청자 본인만·소속 교차 조회·
  내부 전용 댓글의 첨부·사유 없는 반려)를 고정합니다
- **데모 데이터는 늙지 않습니다** — 가상 시드는 만든 날을 오늘로 보고 날짜를 찍어서, 그대로 두면
  한 달 뒤 조회 첫 화면이 0건이 됩니다(라이브에서 실제로 그랬습니다). 데모 시계가 하루 단위로
  세계를 통째로 밀어 늘 "지금 운영 중인" 포털로 보이게 합니다 — 방문자가 만든 데이터는 버리지 않습니다
...
- **접근성·성능은 재서 기록합니다** (`npm run measure`) — 측정한 6개 화면 모두 Lighthouse **접근성 100 ·
