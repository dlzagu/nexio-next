...
쓰기(신청 저장·액션·댓글·첨부)의 허용 여부는 **저장소가 정한다**:
로컬 파일 DB = 허용 · 서버리스 메모리 = 잠금(인스턴스마다 달라 저장이 어긋난다) ·
**서버리스 + 공유 DB(`TURSO_DATABASE_URL`) = 허용**(ADR-0009). `ALLOW_DEV_WRITES` 를 주면 그 값이 이긴다.
잠겨 있으면 화면이 권한 판정까지만 하고 202 로 "여기까지 통과"를 알려준다 (성공 판정은 200 뿐).
...
| `src/lib/data/customers.ts` | 고객사 등록·비활성 — "삭제"는 `ACTIVE='N'` (ADR-0010) |
...
| `src/components/layout/MobileNav.tsx` | 좁은 화면의 주 메뉴 — 사이드바 알맹이(`SidebarBody`)를 **그대로** 서랍에 담는다 |
...
- **원본 필드명을 코드에 들이지 않는다** — `CONTENT`·`ECHONUM`·`PUBLICYN` 같은 함정은 **`src/lib/data/` 경계 한 곳**에서만 변환. 컴포넌트는 정규화된 이름만 본다.
- **zod 스키마 하나를 폼 검증과 라우트 입력 검증에 함께 쓴다** (`src/lib/schemas.ts`).
- **권한은 화면이 아니라 서버가 집행한다** — 가시성(테넌트 격리·비공개)은 `scopeClause`(`data/tickets.ts`)가 SQL 로, 액션은 라우트가 티켓을 다시 읽어 `canDo()` 로 판정한다. 화면의 `canDo()` 는 **버튼을 그릴지 정할 뿐**이라 그것만 믿으면 구멍이 난다.
...
| 참조되는 마스터를 실제로 DELETE 한다 | `ACTIVE='N'` (비활성) | 티켓 수천 건이 고객사 코드를 물고 있어 행을 지우면 **고객사명이 사라진 티켓**이 남는다. 이 앱에는 지우는 경로가 없다 — 쓰기 관문이 DELETE 를 거부한다 |
| `next build` 뒤 같은 폴더에서 `next dev` | 그 사이 `.next` 를 지운다 | 산출물이 섞여 **모든 라우트가 조용히 404** 가 된다. 컴파일 에러도 안 뜨고 레이아웃만 렌더돼 원인을 찾기 어렵다 |
| 페이지가 빌드 시점에 DB 를 읽게 둔다 | 루트 레이아웃에 `force-dynamic` | 쿠키를 안 읽는 페이지는 Next 가 정적 생성 대상으로 잡아 **빌드가 DB 를 호출**한다. 로컬 파일 DB 로는 통과하지만(대신 그 화면이 빌드 시점 데이터로 굳는다) 원격 DB 를 붙이면 배포가 통째로 실패한다 |
| 스크롤 컨테이너 안에 또 스크롤 컨테이너를 둔다 | 스크롤 주체를 **하나만** 정한다 (`Sheet bodyScroll={false}`) | 바깥이 스크롤 컨테이너면 안쪽의 `flex-1 min-h-0` 이 무력화돼 안쪽은 스크롤되지 않는데, `overscroll-behavior: contain` 이 **휠을 바깥으로 넘기지도 않는다** → 스크롤바를 직접 끌어야만 움직인다 |
| 시드에 데이터를 더하고 공유 DB 는 그대로 둔다 | `npm run db:sync:remote` (마스터만 얹는다) | 코드는 push 로 배포되지만 **데모 데이터는 따라오지 않는다.** 로컬 파일 DB 는 `SCHEMA_VERSION` 으로 다시 만들어지는데 공유 DB 엔 그 장치가 없다 → 새 계정이 라이브에서 404. 전체 재시드(`db:seed:remote`)는 라이브에서 만들어진 티켓·댓글·첨부를 통째로 버린다 |
| 선택지 목록을 코드 상수로만 만든다 | 목록을 **DB 와 대조해** 만들고, 없으면 비활성 + 이유 | 목록(코드)과 대상(DB)은 따로 배포돼 언젠가 어긋난다. 어긋난 항목은 눌러도 아무 일이 없는 버튼이 되고, 그건 사용자에게 **고장**으로 보인다 |
| 역할 전환 응답을 버리고 무조건 `router.refresh()` | `res.status === 200` 일 때만 refresh, 아니면 이유를 띄운다 | 거절(404)돼도 화면이 그대로라 원인을 알 수 없다. 쓰기 응답의 202 규칙과 같은 축이다 — **응답을 안 보는 것이 제일 나쁘다** |
| 컴포넌트 클래스(`.btn`·`.seg`)에 `md:hidden` 을 붙인다 | **감싸는 요소**에서 접는다 (`<span className="md:hidden">`) | `globals.css` 의 컴포넌트 클래스는 레이어 밖이라 Tailwind 유틸리티를 **이긴다.** `display` 를 정하는 클래스가 붙어 있으면 `hidden` 이 조용히 무시된다 — 좁은 화면에서 사이드바는 사라졌는데 메뉴 버튼이 넓은 화면에도 남는 식 |
| 좁은 화면에서 표를 가로 스크롤로 민다 | 덜 중요한 열을 **접는다** (`meta.fold`) | 옆으로 밀어야 보이는 열은 있다는 사실조차 모른다. 기준은 "이 열이 없어도 행을 알아볼 수 있는가" — 나머지는 상세에서 본다 |
| 사이드바를 flex 자식으로만 두고 높이를 안 정한다 | `sticky top-0 h-dvh` 로 **화면에 고정** | flex 가 본문 높이만큼 늘려 버린다. 대시보드처럼 긴 화면에서 하단(사용자·보조 메뉴)이 **페이지 맨 아래**로 내려가, 끝까지 스크롤해야 보인다 |
| 단계 진입은 아무나, 그다음 단계는 담당자만 | 진입에서 **담당을 옮기고**(인계), 옮긴 사실을 이력에 남긴다 | 접수는 누구나 되는데 해결안 제시는 담당자만이면, 남의 건을 접수한 사람이 그 자리에서 막힌다 (실측: 접수 대기 26건 중 19건이 이미 배정돼 있었다). ⚠️ 담당을 조용히 옮기면 원래 담당자는 자기 건이 **사라진 것으로만** 안다 |
| 상태만 바꾸는 전이를 허용한다 | 전이표가 **필요한 입력**을 쥔다 (`requires`) | 해결안 제시가 답변 없이 통과하면, 고객 화면은 처리결과 탭이 기본으로 열리는데 **빈 화면**이 뜬다 |

### 원본 시스템에서 관찰한 것 (지금 코드와 무관 — 역사 기록)

Spring 백엔드 연동은 폐기됐다(ADR-0004). 아래는 **원본을 분석하며 확인한 사실**이고
이 저장소의 코드에는 해당하지 않는다 — 규칙으로 읽지 말 것.

| 관찰 | 내용 |
|---|---|
| 세션 | 원본이 `max-sessions=1` 이라 BFF 가 자체 세션을 만들면 사용자 브라우저 세션이 끊겼다 (ADR-0001) |
| 인증 만료 | `401` 이 아니라 `302 → /login` 으로 왔다. 리다이렉트를 따라가면 로그인 HTML 을 200 으로 받는다 |
| 보안 설정 | CSRF 가 사실상 꺼져 있고 `/c4web/**` 가 permitAll 이었다 |
...
- **고객사 관리**: `docs/decisions/ADR-0010` — 내부관리 중 하나만 이식한 이유 · 삭제=비활성 · 권한 2단
- **라이브 쓰기**: `docs/decisions/ADR-0009` — 공유 DB(libSQL) · 저장소가 쓰기 허용을 정한다 · 원격 시드 복사
