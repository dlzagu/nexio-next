...
DB 교체 시 수정 대상은 `src/lib/db.ts` 하나다. 쓰기 허용 여부는 저장소가 정한다(위 명령어 절 · ADR-0009) — 로컬 파일 DB 는 기본 허용.
...
| `src/lib/data/tasks.ts` | 정기 업무 템플릿 — 저장 문장을 **돌려주고** 티켓 트랜잭션에 얹는다 (ADR-0011) |
| `src/components/board/TaskSheet.tsx` | 업무 등록(대리 등록) · 정기 업무 목록 — 보드에서 연다 |
...
| 떠 있는 목록(콤보박스·메뉴)에 토큰 서열을 그대로 쓴다 | `.pop` — 시트·모달보다 **위** | 토큰은 `dropdown 200 < sheet 300 < modal 400` 이라, 시트 안에서 연 드롭다운이 시트 뒤로 깔린다. 열리긴 하는데 안 보이고, 그 자리를 누르면 시트가 클릭을 가져가 팝오버는 '바깥 클릭'으로 닫힌다 → 사용자에게는 **"눌러도 안 열린다"** 로 보인다 (실측: 업무 등록·접수 분류 모달 양쪽) |
| 등록·저장 후 아무 목록으로나 돌려보낸다 | 그 건이 **실제로 걸리는** 뷰로 (`listViewForStage`) | 뷰마다 조건이 다르다 — '진행 중'은 종료건을 빼고 '내 담당'은 신청자·담당자가 나인 것만 담는다. 접수 전(2)으로 등록하면 담당이 없어 '내 담당'이 0건이 되고, 상세만 열린 채 뒤가 비어 **저장이 안 된 것처럼** 보인다 |
| 완료 상태를 상태값만 바꿔 만든다 | 완료일·최종처리자·처리내용을 **함께** 찍는다 | `SUCCDATE` 가 없으면 '최근 완료'(30일)와 보드 완료 컬럼에서 통째로 빠져 **완료로 만들었는데 어디에도 없는** 티켓이 된다. 처리내용이 없으면 처리결과 탭이 기본으로 열리는데 빈 화면이고, 종료건이라 **채워 넣을 수도 없다** |
| 대리 등록한 건을 승인 단계(대기)로 보낸다 | 승인을 건너뛰고 신청·진행에서 시작 | 대기는 **고객사 승인권자만** 풀 수 있다. 우리가 받아 적은 건을 그 줄에 세우면 승인권자는 신청하지도 않은 건을 승인해야 하고, 그때까지 아무도 진행시킬 수 없다 (시드가 같은 실수를 해 8건이 갇혔다) |
| 대리 등록 건을 `MEDIA='포털'` 로 남긴다 | 출처(전화·이메일·내부)와 `REQTYPE`(SERVICE/WORK)을 함께 남긴다 | 포털로 들어온 것처럼 보이면 "이 건 어디서 왔나"에 영원히 답할 수 없다. 이력 첫 줄에도 누가 대신 등록했는지 남긴다 |
| 반복 업무를 티켓의 플래그로 만든다 | 템플릿 표를 따로 두고 **이번 달 실체**만 티켓으로 (ADR-0011) | 티켓에 달면 종료된 건이 계속 살아 있는 셈이 된다. 템플릿 저장은 티켓과 한 트랜잭션 — 나눠 커밋하면 다음 달이 조용히 빈다 |
| 반복 기준일을 29~31일로 받는다 | 1~28일만 | 없는 달이 생겨 "이번 달에는 안 만들어지는 정기 업무"가 조용히 만들어진다 |
| 개발 서버를 켠 채 `npm run db:reset` 하고 지워졌다고 믿는다 | 서버를 멈추고 실행한다 (스크립트가 EBUSY 를 알려준다) | 잠긴 파일에 `fs.rmSync` 를 걸면 Node 가 출력 없이 죽는다(0xC0000409). 옛 데이터가 그대로인 채 다음 작업이 굴러가고, 실제로 그 상태로 README 스크린샷을 찍어 시드에 없는 데이터가 찍혔다 |
| 새 표를 추가하고 공유 DB 는 그대로 둔다 | `npm run db:sync:remote` 가 **없는 표를 만든다** | 로컬은 `SCHEMA_VERSION` 으로 다시 만들어지지만 공유 DB 엔 그 장치가 없다 → 배포 순간 라이브만 `no such table` 로 500 |
...
| 가상 시드의 날짜를 한 번 찍고 그대로 둔다 | 데모 시계가 **민다** (`NX_DEMO_CLOCK`, ADR-0012). 말뭉치에 달력 날짜를 쓰지 않는다 | 달력만 흘러 한 달 뒤 조회 첫 화면('내 담당 · 최근 15일')이 **모든 방문자에게 0건**이었다(라이브 실측). 배포는 green 이라 아무것도 경보를 울리지 않는다 |
| 저장할 것이 없는 '저장'에 200 을 준다 | `400 NOTHING_TO_SAVE` — 성공은 **실제로 바뀐 것**이 있을 때만 | 액션바 '저장'이 초안을 안 싣고 보냈는데 200 "저장했습니다" → 화면이 초안을 버려 쓰던 답변이 **사라졌다**(DB 는 null — Aside 실측) |
| 리치 에디터를 `value` 로 **처음 한 번만** 채운다 | 부모 값이 바뀌면 따라간다 (되돌아온 자기 값은 무시 — `emitted` ref) | 댓글을 등록해도 글이 입력창에 남고, 이어 쓰면 **앞 글까지 다시 등록**된다. 저장 후 초기화된 초안과 화면 글이 갈라진다 |
| 저장 시각을 서버 로컬 시계로 찍는다 (`format(new Date())`) | `toDbStamp()` = 한국 벽시계. 채번 연월도 같은 스탬프에서 | 개발 PC(KST)에선 맞아 보이는데 Vercel(UTC)의 **라이브 쓰기만** 9시간 이르게 찍혀, 오전 댓글이 전날 밤 글이 되고 월초 새벽 신청이 지난달 번호를 받는다 |
| 저장된 HTML 을 무조건 먼저 디코드한다 | `unescapeStoredMarkup` — 진짜 태그가 **없고** 이스케이프 태그만 있을 때만 | 평문은 이스케이프해 `<p>` 로 감싸 저장한다. 무조건 풀면 사용자가 쓴 `<select>`·`a<b` 가 태그가 되어 미리보기·재신청 프리필에서 지워진다 |
| "앱이 안 쓰는 표"라고 믿고 표 전체를 시드 시점의 증인으로 읽는다 | 시드가 만든 행만 — **보이는 공지 · 번호 ≤ 시드 공지 수** (`LEGACY_WITNESS_SQL`) | `BOARD_DETAIL` 에는 고객사 관리가 숨김 이력 행을 '지금' 날짜로 쓴다. 그 행을 증인으로 읽으면 데모 시계 기준이 미래로 잡혀 한 달 늙은 라이브가 **영영 못 따라잡는다**(리뷰 재현). 같은 이유로 sync 도 숨김 행을 번호로 덮으면 안 된다 |
| 되돌릴 수 없는 액션(반려·취소·계속 진행)을 1클릭으로 실행 | 확인 모달 + 사유(반려·계속 진행·취소 권유는 필수 — 서버도 400) | 잘못 누르면 사유 없는 '요청이 반려되었습니다'만 남고 되돌릴 길이 없다. 보드의 취소요청 카드는 '진행 ›' 한 번에 고객의 취소 요청을 거절하고 있었다 |
...
- **데모 시계**: `docs/decisions/ADR-0012` — 시드가 달력과 함께 늙지 않게 **민다**(재시드 아님) ·
  불변식 4개(미래 금지·요청 단위·이상치 보존·이중 이동 금지) · 옛 공유 DB 는 sync 한 번
- **업무 등록**: `docs/decisions/ADR-0011` — 대리 등록(전화·메일·정기) · 출처 기록 ·
  승인 줄에 세우지 않는 이유 · 정기 업무 템플릿
